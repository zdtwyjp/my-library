   ...
   <script type="text/javascript">
	...
	Ext.onReady(function()
	{
	    ...
	    var store = new Ext.data.Store({
	        ...
	        proxy:new Ext.data.HttpProxy({url:'<%=path%>/console/data/setting/initCountryGroups.htm'}),
			...
			tbar: new Ext.Toolbar({
        		items:[
        			'组团名称:',{
        				...
        				enableToggle: false,
        				iconCls:'common_add',
        				handler: countryGroupAdd
        			},'-',{
        				text: '&nbsp;编&nbsp;辑&nbsp;',
        				enableToggle: false,
        				iconCls:'common_edit',
        				handler: countryGroupEdit
        			},'-',{
		             	text: '&nbsp;删&nbsp;除&nbsp;',
        				enableToggle: false,
        				     ...
        				     if(snIds !=""){
        				     		Ext.util.Format.trim(snIds);
        				              Ext.MessageBox.show({
									        ...
									        fn: function(e) {
									        	if(e == 'yes') {
									        		deleteCountryGroup(snIds);
									        	}
									        },
									        icon : Ext.MessageBox.QUESTION
        				              });
        				     }else{
        				    	 alertWarring("请选择需要删除的组团！");
        				     }		
        				}
        			}
        		]
		...
		function saveForm() {
			if(add_Form.form.isValid()) {
				add_Form.getForm().submit({
					url : '<%=path%>/console/data/setting/saveCountryGroup.htm',
					method : 'post',
					params : {
						isEditing : isEditing,
		...
		function deleteCountryGroup(snIds){
			Ext.Ajax.request({
				url : '<%=path%>/console/data/setting/delCountryGroups.htm',
				params : {
					snIds : snIds
				},
