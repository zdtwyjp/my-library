   ...
   <script type="text/javascript">
	...
	Ext.onReady(function()
	{
	    ...
	    var store = new Ext.data.Store({
	        ...
	        proxy:new Ext.data.HttpProxy({url:'<%=path%>/console/data/setting/initCitys.htm'}),
		...
		var add_Form = new Ext.form.FormPanel({
			...
			items : [province_Combox,name_Add,serialNumber_add],
			buttons : [{
				text : '保&nbsp;存',
				handler : saveForm
			}, {
		...
		function saveForm() {
			if(add_Form.form.isValid()) {
				add_Form.getForm().submit({
					url : '<%=path%>/console/data/setting/saveCitys.htm',
					method : 'post',
					params : {
						isEditing : isEditing,
		...
		function deleteCitysFun(snIds){
			Ext.Ajax.request({
				url : '<%=path%>/console/data/setting/delCitysFun.htm',
				params : {
					snIds : snIds
				},
